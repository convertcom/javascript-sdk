...
export type LocationTriggerCallback = LocationTriggerBase & {
    type?: 'callback';
    /**
     * Describes js callback to execute, code will be wrapped into a function with 2 arguments:
     * - `activate` which is a function that needs to be called to activate the location
     * - `options` which is an object with the following properties:
     * - `options.locationId` - id of the location that is being activated
     * - `options.isActive` - boolean flag that indicates if the location is active
     *
     */
    js: string;
};
